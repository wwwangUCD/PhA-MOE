...
Use two steps backward

When training on PACE data, we suggest to use
models.MDN_MoE_s_new import *, instead of using models.MDN_MoE_s import *
because the new version is more robust to Singular Matrix Error where MDN trys to reverse a singular matrix.
"""
import sys
...
import math
def main(system,numbers,hidden=[100] * 5, band_width = 10, num_experts = 6,k = 4, gamma = 100,seed=42,
    ...
    save_dir = f'../Results_Feb21/{system}/{folder_name}/MDN_MOE_s/seed_{seed}'
    ...
    for epoch in range(num_epochs):
        ...
        avg_valid_loss = evaluate_model(model, val_loader, MDN_criterion, device,gamma=gamma)
        ...
        if (epoch + 1) % 100 == 0:
            print(f'Epoch [{epoch + 1}/{num_epochs}], Train Loss: {avg_train_loss:.4f}, Valid Loss: {avg_valid_loss:.4f}, Train MoE loss {gamma * Moe_loss}')
    ...
    all_targets, all_outputs, avg_test_loss = evaluate_model(model, test_loader, MDN_criterion, device, record_outputs=True)
    ...
    all_targets, all_outputs, avg_val_loss = evaluate_model(model, val_loader, MDN_criterion, device, record_outputs=True)
    if all_outputs.dim() == 3:
        all_outputs = all_outputs.squeeze(1)
    all_targets_np = all_targets.numpy()
    all_outputs_np = all_outputs.numpy()
...
if __name__ == "__main__":
    ...
    for system in systems:
        if system == 'HICO':
            ...
            hidden_mult=[1] * 5  # Example: Slightly varied pyramid
            neuron = 256
            band_width = 52
            num_experts = 8
            k = 4
            ...
            num_experts = 8
            k = 6
            seeds = list(range(42, 53))  # for the PACE case, needs to run the 53 to 72
        elif system == 'PACE':
            numbers = [
            ...
            hidden_mult=[1] * 6  # Example: Slightly varied pyramid
            neuron = 256
            band_width = 144
            num_experts = 8
            k = 4
            seeds = list(range(42, 72))  # for the PACE case, needs to run the 53 to 72
        else:
        ...
        hidden = [neuron * m for m in hidden_mult]
        # Initialize the progress bar
        with tqdm(total=total_iterations, desc="Total Progress") as pbar:
            for seed in seeds:
                for params in parameter_combinations:
                    start_time = time.time()
                    main(system,numbers,hidden=hidden, band_width=band_width, num_experts=num_experts, k=k, gamma=1, seed=seed,

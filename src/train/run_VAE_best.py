...
def main(system,numbers,hidden_size=32, mult_e=[4,2],mult_d=[2,4], seed=42,
    ...
    depth = len(mult_e)+len(mult_d)
    neuron=hidden_size
    kld_weight=0.01
    folder_name = f"{scaler_x_type}_{scaler_x_scope}_{scaler_y_type}_{scaler_y_scope}"
    save_dir = f'../Results_Mar/{system}/{folder_name}/VAE_1000/seed_{seed}'
    ...
    model = VAE(input_size=input_size, hidden_size=neuron, output_size=output_size,mult_e=mult_e, mult_d=mult_d, activation='relu', use_batchnorm=True)
    ...
    for epoch in range(num_epochs):
        ...
        avg_valid_loss = evaluate_model(model, val_loader, device, kld_weight=kld_weight)
        valid_losses.append(avg_valid_loss)
        avg_test_loss = evaluate_model(model, test_loader, device, kld_weight=kld_weight)
    ...
    all_targets, all_outputs, avg_test_loss = evaluate_model(model, test_loader, device, record_outputs=True, kld_weight=kld_weight)
    ...
    all_targets, all_outputs, avg_val_loss = evaluate_model(model, val_loader, device, record_outputs=True, kld_weight=kld_weight)
    if all_outputs.dim() == 3:
        all_outputs = all_outputs.squeeze(1)
    all_targets_np = all_targets.numpy()
    all_outputs_np = all_outputs.numpy()
...
if __name__ == "__main__":
    ...
    for system in systems:
        if system == 'HICO':
            ...
            # mult_e = [2, 1] # in the paper we use this one.
            mult_e = [2, 1, 1] # this reuslt is not used
            mult_d = [1, 1]
        ...
        with tqdm(total=total_iterations, desc="Total Progress") as pbar:
                for seed in seeds:
                    for params in parameter_combinations:
                        ...
                        main(system,numbers,hidden_size=neuron, mult_e=mult_e, mult_d=mult_d, seed=seed,

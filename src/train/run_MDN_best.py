...
import math
def main(system,numbers,hidden=[100] * 5, seed=42,
    ...
    save_dir = f'../Results_Feb21/{system}/{folder_name}/MDN/seed_{seed}'
    ...
    for epoch in range(num_epochs):
        try:
            ...
            avg_valid_loss = evaluate_model(model, val_loader, MDN_criterion, device)
            valid_losses.append(avg_valid_loss)
            avg_test_loss = evaluate_model(model, test_loader, MDN_criterion, device)
            ...
            print(f"Error occurred during training at epoch {epoch}: {e}")
            print("Stopping training for this model.")
            break  # Exit the training loop for this model
        # best_valid_loss = avg_valid_loss
        # best_epoch = epoch
        # best_train_loss = avg_train_loss
        # best_test_loss = avg_test_loss
...
if __name__ == "__main__":
    ...
    # Generate the hidden layer combinations
    hidden_combinations = []
    for neuron in neurons:
        for mult in hidden_mults:
            hidden_combinations.append([neuron * m for m in mult])
    ...
    total_iterations = len(hidden_combinations) * len(seeds)*len(parameter_combinations)

    # Initialize the progress bar
    with tqdm(total=total_iterations, desc="Total Progress") as pbar:
        for params in parameter_combinations:
            for hidden in hidden_combinations:
                for seed in seeds:
                    start_time = time.time()
                    main(system,numbers,hidden=hidden, seed=seed,

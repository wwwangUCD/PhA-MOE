...
folder = 'pp'
# filename = 'Aphy_WB_pp.pdf'
filename = 'Aphy_no_pp.pdf'
...
# Assuming selected_wl and Aphy_selected are already defined
...
# Create the boxplot for the Aphy_selected data
ax.boxplot(Aphy_selected, positions=selected_wl, vert=True, patch_artist=True, widths=6,
...
for patch in ax.artists:
    patch.set_facecolor('lightgreen')  # Different color for Aphy data
...
ax.set_ylabel(r'$a_{phy}(m^{-1})$', fontsize=fontsize)
# ax.set_title(r'Box plots of $a_{phy}$ data values across wavelengths', fontsize=fontsize)
